...
public class DigActionPerformer implements ModAction, ActionPerformer {

    private final ActionEntry actionEntry;
    private final int actionId;

    DigActionPerformer(int actionId, ActionEntry actionEntry) {
        this.actionEntry = actionEntry;
        this.actionId = actionId;
    }

    private static class SingletonHelper {
        private static final DigActionPerformer _performer;
        static {
            _performer = new DigActionPerformer(Actions.DIG, Actions.actionEntrys[Actions.DIG]);
        }
    }

    static DigActionPerformer getDigActionPerformer(){
    ...
    public boolean action(Action action, Creature performer, Item source, int tileX, int tileY, boolean onSurface, int heightOffset,
        ...
        DigAction terraformerDig;
        if (!DigAction.hashMapHasInstance(action)) {
            ...
            terraformerDig = new DigAction(action, performer, source, SkillList.DIGGING, options.getMinSkill(),
            ...
            terraformerDig = DigAction.actionDataWeakHashMap.get(action);
