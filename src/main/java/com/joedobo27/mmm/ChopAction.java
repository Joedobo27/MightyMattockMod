...
class ChopAction extends ActionMaster {

    private final TilePos targetedTile;
    private final ArrayList<Function<ActionMaster, Boolean>> failureTestFunctions;

    static WeakHashMap<Action, ChopAction> actionDataWeakHashMap = new WeakHashMap<>();

    ChopAction(Action action, Creature performer, @Nullable Item activeTool, int usedSkill, int minSkill,
    ...
    double doSkillCheckAndGetPower(float counter) {
        ...
        Tiles.Tile tileType = Tiles.getTile(Tiles.decodeType(encodedTile));
        if (tileType == null)
            return -1.0d;
        if (tileType.isBush()) {
            BushData.BushType bushType = BushData.BushType.fromTileData(Tiles.decodeTileData(encodedTile));
            difficulty = bushType.getDifficulty();
        }
        if (tileType.isTree()) {
            TreeData.TreeType treeType = TreeData.TreeType.fromTileData(Tiles.decodeTileData(encodedTile));
    ...
    Item makeFelledTreeOnGround(double power) {
        ...
        if (this.activeTool != null && this.activeTool.getSpellEffects() != null &&
                this.activeTool.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_RESGATHERED) != -10L) {
