...
public class ChopAction implements ModAction, ActionPerformer {
    ...
    ChopAction(int actionId, ActionEntry actionEntry) {
    ...
    public boolean action(Action action, Creature performer, Item activeItem, int tileX, int tileY, boolean onSurface,
        ...
        if (!ChopTerraformer.hashMapHasInstance(action)) {
            ...
            chopTerraformer = new ChopTerraformer(action, performer, activeItem, SkillList.WOODCUTTING,
            ...
            chopTerraformer = ChopTerraformer.actionDataWeakHashMap.get(action);
