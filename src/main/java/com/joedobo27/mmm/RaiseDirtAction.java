...
class RaiseDirtAction extends ActionMaster {

    private final TilePos targetedTile;
    private final int actionId;
    private final ArrayList<Function<ActionMaster, Boolean>> failureTestFunctions;
    static WeakHashMap<Action, RaiseDirtAction> actionDataWeakHashMap = new WeakHashMap<>();

    RaiseDirtAction(Action action, Creature performer, @Nullable Item activeTool, int usedSkill, int minSkill,
    ...
    void destroyRaiseResource() {
        Item[] items = this.getInventoryItems(ItemList.dirtPile);
        if (items == null || items.length == 0) {
            items = this.getGroundItems(ItemList.dirtPile, this.targetedTile);
        }
        if (items == null)
            return;
